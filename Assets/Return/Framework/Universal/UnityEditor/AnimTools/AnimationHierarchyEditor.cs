...
public class AnimationHierarchyEditor : EditorWindow
{
	...
	private string sOriginalRoot = "CharacterTransform";
	private string sNewRoot = "SomeNewObject/CharacterTransform";
	...
	void OnGUI()
	{
		...
		if (animationClips.Count > 0)
		{
			...
			GUILayout.Label("Referenced Animator (CharacterTransform):", GUILayout.Width(columnWidth));
			...
			if (GUILayout.Button("Replace CharacterTransform"))
			{
	...
	void ReplaceRoot(string oldRoot, string newRoot)
	{
		...
		for (int iCurrentClip = 0; iCurrentClip < animationClips.Count; iCurrentClip++)
		{
			...
			Undo.RecordObject(animationClip, "Animation Hierarchy CharacterTransform Change");
	...
	void UpdatePath(string oldPath, string newPath)
	{
		...
		{
			throw new UnityException("m_Path " + newPath + " already exists in that animation!");
		}
		AssetDatabase.StartAssetEditing();
		for (int iCurrentClip = 0; iCurrentClip < animationClips.Count; iCurrentClip++)
		{
	...
	string ChildPath(GameObject obj, bool sep = false)
	{
		if (animatorObject == null)
		{
			throw new UnityException("Please assign Referenced Animator (CharacterTransform) first!");
		}
