...
public sealed class MotionAnimation
{
    ...
    /// <summary>
    /// Provides a set of rules used to simulate the walking animation on the Target ReadOnlyTransform.
    /// </summary>
    [SerializeField]
    [Tooltip("Provides a set of rules used to simulate the walking animation on the Target ReadOnlyTransform.")]
    ...
    /// <summary>
    /// Provides a set of rules used to simulate the walking animation on the Target ReadOnlyTransform (Used when the character has their legs injured).
    /// </summary>
    [SerializeField]
    [Tooltip("Provides a set of rules used to simulate the walking animation on the Target ReadOnlyTransform (Used when the character has their legs injured).")]
    private MotionData m_BrokenLegsMotionData;

    /// <summary>
    /// Provides a set of rules used to simulate the running animation on the Target ReadOnlyTransform.
    /// </summary>
    [SerializeField]
    [Tooltip("Provides a set of rules used to simulate the running animation on the Target ReadOnlyTransform.")]
    ...
    /// <summary>
    /// Defines the how much the Target ReadOnlyTransform will be affected by this animation by increasing the animation amplitude.
    /// </summary>
    [SerializeField]
    [Tooltip("Defines the how much the Target ReadOnlyTransform will be affected by this animation by increasing the animation amplitude.")]
    ...
    /// <summary>
    /// The overall speed of this anim.
    /// </summary>
    ...
    /// </summary>
    public void MovementAnimation(dynamic FPController)
    {
        if (!m_TargetTransform)
            return;

        // Stops the movement animation if the character is not walking or running.
        //if (FPController.State == ObsoleteMotionState.Flying || FPController.State == ObsoleteMotionState.IdleState || FPController.State == ObsoleteMotionState.Climbing || FPController.IsSliding)
        //{
        //    PerformMovementAnimation(null, FPController.ModuleVelocity, 10);
        //}
        //else
        {
            // Calculates the animation speed
            //float speed = Mathf.Max(FPController.State == ObsoleteMotionState.Running ? 6.5f : 2.75f, FPController.CurrentTargetForce);

            // Lowerbody damaged (broken legs)
            //if (FPController.LowerBodyDamaged && !FPController.IsAiming)
            //{
            //    PerformMovementAnimation(m_BrokenLegsMotionData, FPController.Grounded ? FPController.ModuleVelocity : Vector3.zero, speed);
            //}
            //else
            //{
            //    switch (FPController.State)
            //    {
            //        case ObsoleteMotionState.Running:
            //            PerformMovementAnimation(m_RunningMotionData, FPController.Grounded ? FPController.ModuleVelocity : Vector3.zero, speed);
            //            break;
            //        case ObsoleteMotionState.Crouched:
            //            PerformMovementAnimation(m_CrouchedMotionData, FPController.Grounded ? FPController.ModuleVelocity : Vector3.zero, speed);
            //            break;
            //        case ObsoleteMotionState.Walking:
            //            PerformMovementAnimation(m_WalkingMotionData, FPController.Grounded ? FPController.ModuleVelocity : Vector3.zero, speed);
            //            break;
            //    }
            //}
        }
    ...
    /// <param name="overallSpeed">The overall anim speed.</param>
    ...
    {
        Vector3 initialRot = MathfUtility.RandomInsideBounds(m_MinHitRotation, m_MaxHitRotation);
